...
class Site extends MY_Controller {
    ...
    function contact() {
        if ($_POST) {
            ...
            $this->email->to('mayur.panchal@searchnative.in');
    ...
    function subscriber() {
        ...
        if ($is_subscriber) {
            ...
            $this->email->from('mayur.panchal@searchnative.in', 'Mayur Panchal (Searchnative India Pvt. Ltd)');
    ...
    function forgot_password() {
        if ($_POST) {
            ...
            if ($record) {
                ...
                $this->load->library('email', $config);
                $this->email->set_newline("\r\n");
                $this->email->from('mayur.panchal@searchnative.in', 'Learning Management System');
                $this->email->to($_POST['email']);
...
SELECT STR_TO_DATE(todo_datetime,'%Y-%m-%d') as todo_date FROM todo_list)")->result();
    ...
    function fake_insert()
    {
        ...
        echo "<br>";
        echo "Course ".$course ='5';
        echo "<br>";
        echo "Class ".$class  ='2';
        echo "<br>";
        echo "Admission Plan ".$admission_plan = '4';
        echo "<br>";
        echo "<br>";
        echo "<br>";
        ...
        for ($i = 1; $i <= 30; $i++) {
          ...
          $std_id = $this->db->insert('student', array(
                'user_id' => $user_id,
            ...
            echo $std_id;
             $roll_number = date('Y').$branch.$course.$std_id;
          $res = $this->db->get_where('student',array("std_id"=>$std_id))->result();        
